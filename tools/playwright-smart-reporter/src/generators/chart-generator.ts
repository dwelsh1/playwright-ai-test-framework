...
import { formatDuration, formatShortDate, computePercentile } from '../utils';
...
export function generateTrendChart(data: ChartData): string {
  ...
  const currentFlaky = data.results.filter(
  ...
  // Compute current run p90 per-test duration
  const MIN_SAMPLE = 3;
  const nonSkippedDurations = data.results
    .filter((r) => r.status !== 'skipped')
    .map((r) => r.duration)
    .sort((a, b) => a - b);
  const currentP90Ms =
    nonSkippedDurations.length >= MIN_SAMPLE
  ...
  // Compute p90 per-test chart data — include current run
  const p90Summaries: RunSummary[] = allSummaries.map((s, i) =>
    i === allSummaries.length - 1 && currentP90Ms !== undefined
      ? { ...s, p90Duration: currentP90Ms }
      : s,
  );
  const hasP90Data = p90Summaries.some((s) => (s.p90Duration ?? 0) > 0);
  const maxP90Ms = Math.max(...p90Summaries.map((s) => s.p90Duration ?? 0), 1);
  const maxP90s = Math.ceil(maxP90Ms / 1000);
  ...
  const generateBarChart = (
    ...
    const bars = chartData
      .map((d, i) => {
        ...
        const branch = escapeHtml(d.metadata?.build?.branch ?? '');
        const env = escapeHtml(d.metadata?.build?.environment ?? '');

        return `
        <g class="bar-group${clickable ? ' clickable' : ''}" data-tooltip="${label}: ${displayValue}${isAnomaly ? ' ⚠ Anomaly' : ''}" ${clickable ? `data-runid="${escapeHtml(runId)}" onclick="loadHistoricalRun('${safeRunId}', '${label}')"` : ''} data-branch="${branch}" data-env="${env}">
          <rect
            x="${x}"
            y="${y}"
    ...
    // Generate optional horizontal reference line (e.g. p90 budget)
    const refLineHtml =
      referenceLine && referenceLine.value <= maxValue
        ? (() => {
            const ry = (
              padding.top +
              plotHeight -
              (referenceLine.value / maxValue) * plotHeight
            ).toFixed(1);
            return `
        <line x1="${padding.left}" y1="${ry}" x2="${padding.left + plotWidth}" y2="${ry}"
              stroke="${referenceLine.color}" stroke-width="1.5" stroke-dasharray="5 3" opacity="0.8"/>
        <text x="${padding.left + plotWidth - 4}" y="${(parseFloat(ry) - 3).toFixed(1)}"
              fill="${referenceLine.color}" font-size="9" text-anchor="end">${referenceLine.label}</text>
          `;
          })()
        : '';
  ...
  // Generate p90 per-test duration chart (only when data is available)
  const budgetRefLine =
    data.durationBudgetMs !== undefined
      ? { value: Math.round(data.durationBudgetMs / 1000), color: '#ef4444', label: 'budget' }
      : undefined;
  const p90Chart = hasP90Data
    ? generateBarChart(
        p90Summaries,
        (s) => Math.round((s.p90Duration ?? 0) / 1000),
        maxP90s,
        '#06b6d4',
        'p90 (s)',
        (val) => `${val}s`,
      ...
      <div class="section-content all-charts-grid">
        ...
        ${
          hasP90Data
            ? `
        <div class="line-chart-container">
          <h4 class="chart-title">📐 p90 Test Duration${data.durationBudgetMs !== undefined ? ` <span style="font-size:0.75em;color:#ef4444">(budget: ${formatDuration(data.durationBudgetMs)})</span>` : ''}</h4>
          ${p90Chart}
        </div>
        `
            : ''
