...
export function generateComparison(comparison: RunComparison): string {
  ...
  const durationPctChange =
    baselineRun.duration > 0 ? ((durationDelta / baselineRun.duration) * 100).toFixed(1) : '0';
...
function generateComparisonItem(test: TestResultData, type: string): string {
  ...
  if (type === 'regression' || type === 'improvement') {
    ...
    const pctChange =
      avgDuration > 0 ? (((test.duration - avgDuration) / avgDuration) * 100).toFixed(1) : '0';
    details = `
...
function getColorClass(type: string): string {
  switch (type) {
    ...
    case 'regression':
      return 'regression-section';
    case 'improvement':
      return 'improvement-section';
    case 'new':
      return 'new-section';
    default:
      return '';
...
export function buildComparison(
  ...
  for (const test of currentTests) {
    ...
    if (
      test.status === 'passed' &&
      (baselineTest.status === 'failed' || baselineTest.status === 'timedOut')
    ) {
