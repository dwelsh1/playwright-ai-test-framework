...
export class NetworkCollector {
        ...
        try {
          ...
          if (parsed.type === 'resource-snapshot' && parsed.snapshot) {
            ...
            if (entry && this.shouldInclude(entry)) {
              ...
              result.summary.byStatus[statusGroup] =
                (result.summary.byStatus[statusGroup] || 0) + 1;
              result.summary.byMethod[entry.method] =
                (result.summary.byMethod[entry.method] || 0) + 1;
  ...
  private shouldInclude(entry: NetworkLogEntry): boolean {
    ...
    if (this.options.excludeStaticAssets) {
      ...
      if (
        entry.contentType &&
        STATIC_CONTENT_TYPES.some((ct) => entry.contentType!.startsWith(ct))
      ) {
        return false;
      }
    }
