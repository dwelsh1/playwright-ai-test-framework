...
import type { CIInfo, ReportMetadata, CustomMetadataField, SavedView, LinkedIssue } from './types';
...
function detectCIInfo(): CIInfo | undefined {
  ...
  if (env.GITHUB_ACTIONS) {
    return {
      provider: 'github',
      branch:
        env.GITHUB_HEAD_REF || env.GITHUB_REF_NAME || env.GITHUB_REF?.replace('refs/heads/', ''),
...
class SmartReporter implements Reporter {
  ...
  private customFields: CustomMetadataField[] = [];
  ...
  constructor(options: SmartReporterOptions = {}) {
    ...
    this.aiAnalyzer = new AIAnalyzer({
      ...
      maxTokensForAI: options.smartReporterMaxTokens,
    });
  }
  ...
  onBegin(config: FullConfig, _suite: Suite): void {
    ...
    // Auto-detect CI environment (legacy — kept for backwards compat with ciInfo)
    this.ciInfo = detectCIInfo() ?? { provider: 'local' };

    // Always capture runtime environment metadata
    this.ciInfo.nodeVersion = process.version;
    this.ciInfo.platform = process.platform;
    try {
      const pwPkgPath = require.resolve('@playwright/test/package.json');
      const pwPkg = JSON.parse(fs.readFileSync(pwPkgPath, 'utf-8')) as { version?: string };
      if (pwPkg.version) this.ciInfo.playwrightVersion = pwPkg.version;
    } catch {
      // not critical
    }

    // Collect project & build metadata (Phase 1)
    this.metadataCollector = new MetadataCollector(this.outputDir);
    const metaResult = this.metadataCollector.collect(this.options.metadata);
    this.reportMetadata = metaResult.reportMetadata;
    this.customFields = metaResult.customFields;
    this.savedViews = metaResult.savedViews;
    // outputFile from settings file takes priority over reporter options (allows per-config output paths).
    // Resolve to absolute using process.cwd() — config.rootDir can point to testDir when invoked via
    // --reporter CLI flag, so we must anchor to the actual working directory instead.
    if (metaResult.outputFile) {
      this.options.outputFile = path.resolve(process.cwd(), metaResult.outputFile);
    }
    ...
    const performanceThreshold =
      thresholds?.performanceRegression ?? this.options.performanceThreshold ?? 0.2;
  ...
  async onTestEnd(test: TestCase, result: TestResult): Promise<void> {
    ...
    // Collect custom metadata from annotations matching customFields schema (Phase 1)
    // e.g. test.info().annotations.push({ type: 'domain', description: 'payments' })
    const customMetadata: Record<string, string> = {};
    if (this.customFields.length > 0) {
      const customFieldKeys = new Set(this.customFields.map((f) => f.key));
      for (const a of test.annotations) {
        if (customFieldKeys.has(a.type) && a.description) {
          // Q5: warn if value not in allowed values list
          const field = this.customFields.find((f) => f.key === a.type);
          if (field?.values && !field.values.includes(a.description)) {
            console.warn(
              `[smart-reporter] Unexpected value "${a.description}" for custom field "${a.type}" in ${test.location.file}. Allowed values: ${field.values.join(', ')}.`,
            );
          }
          customMetadata[a.type] = a.description;
        }
      }
    }

    // Parse type: 'issue' annotations into structured linked issues (Phase 1)
    const linkedIssues: LinkedIssue[] = [];
    for (const a of test.annotations) {
      if (a.type === 'issue' && a.description) {
        const ref = a.description.trim();
        linkedIssues.push({
          ref,
          resolvedUrl: ref.startsWith('http') ? ref : undefined,
        });
      }
    }
    ...
    const testData: TestResultData = {
      ...
      // Custom metadata from annotations matching customFields schema
      customMetadata: Object.keys(customMetadata).length > 0 ? customMetadata : undefined,
      // Linked issues from type: 'issue' annotations
    ...
    // Store video paths as relative to the report HTML file so links work in both modes:
    // - HTTP (npm run report:smart:serve): browser resolves relative URL from the serve root
    // - file:// (report opened directly): browser resolves relative URL from the HTML file location
    if (attachments.videos.length > 0) {
      const reportHtmlDir = path.dirname(
        path.resolve(this.outputDir, this.options.outputFile ?? 'smart-report.html'),
      );
      const toRelative = (v: string) =>
        path.relative(reportHtmlDir, path.resolve(v)).split(path.sep).join('/');
      testData.videoPath = toRelative(attachments.videos[0]!);
      attachments.videos = attachments.videos.map(toRelative);
    }

    // Normalize custom attachment paths to be relative to the report HTML dir so links work
    // in both HTTP-served and file:// modes (same reason as videos above)
    if (attachments.custom && attachments.custom.length > 0) {
      const reportHtmlDir = path.dirname(
        path.resolve(this.outputDir, this.options.outputFile ?? 'smart-report.html'),
      );
      attachments.custom = attachments.custom.map((c) => ({
        ...c,
        path: c.path
          ? path.relative(reportHtmlDir, path.resolve(c.path)).split(path.sep).join('/')
          : c.path,
      }));
    }
  ...
  async onEnd(result: FullResult): Promise<void> {
    ...
    if (options.enableComparison !== false) {
      ...
      if (baselineRun) {
        ...
        comparison = buildComparison(this.results, currentSummary, baselineRun, baselineTests);
      }
    }

    const outputPath = path.resolve(this.outputDir, this.options.outputFile ?? 'smart-report.html');

    // Ensure the output directory exists (it may not when outputFile points to a new path)
    fs.mkdirSync(path.dirname(outputPath), { recursive: true });

    // Artifact retention cleanup — delete orphaned screenshot/trace files from prior runs
    // Default: keep only files from the current run (artifactRetentionRuns === 1 or unset)
    // Set artifactRetentionRuns: 0 to disable cleanup entirely.
    const artifactRetentionRuns = this.options.artifactRetentionRuns ?? 1;
    if (artifactRetentionRuns > 0) {
      this.cleanOrphanedArtifacts(path.dirname(outputPath), this.results);
    }
    ...
    const traceResults = this.results.filter(
      (r) => r.attachments?.traces && r.attachments.traces.length > 0,
    );
    ...
    if (this.options.enableHistoryDrilldown) {
      try {
        ...
        const historyPath = path.resolve(
          this.outputDir,
          this.options.historyFile ?? 'test-history.json',
        );
    ...
    // Derive playwrightProjects from collected results
    const playwrightProjects = [
      ...new Set(this.results.map((r) => r.project).filter((p): p is string => !!p)),
    ];
    if (this.reportMetadata) {
      this.reportMetadata.playwrightProjects = playwrightProjects;
    }
    ...
    const servePath = path.join(__dirname, 'bin', 'serve.js');
    if (fs.existsSync(servePath)) {
      console.log(`   To view traces, run: node "${servePath}" "${outputPath}"`);
    } else {
      console.log(
        `   To view traces, run from project root: npm run report:serve -- "${outputPath}"`,
      );
    }
    console.log(`   (Opening as file:// will not load trace files; use the serve command above.)`);
    console.log(`   Or open directly: open "${outputPath}"`);

    // Update history (Phase 5: persist metadata with each run summary)
    this.historyCollector.updateHistory(this.results, this.reportMetadata);
  ...
  /**
   * Delete screenshot and trace files in the output directory that are not
   * referenced by any result in the current run, to prevent unbounded accumulation.
   */
  private cleanOrphanedArtifacts(outputDir: string, results: TestResultData[]): void {
    // Build set of screenshot filenames currently referenced by results
    const referencedScreenshots = new Set<string>();
    for (const r of results) {
      for (const s of r.attachments?.screenshots ?? []) {
        if (s && !s.startsWith('data:')) {
          referencedScreenshots.add(path.basename(s));
        }
      }
    }

    // Build set of trace filenames that WILL be written for this run
    // (traces haven't been copied yet when this runs, so derive the expected filenames)
    const referencedTraces = new Set<string>();
    for (const r of results) {
      const traces = r.attachments?.traces ?? [];
      for (let i = 0; i < traces.length; i++) {
        if (traces[i] && !traces[i].startsWith('./traces/')) {
          // original path — derive the destination filename using the same rule as the copy loop
          const safeTestId = sanitizeFilename(r.testId);
          referencedTraces.add(`${safeTestId}-trace-${i}.zip`);
        } else if (traces[i]?.startsWith('./traces/')) {
          referencedTraces.add(path.basename(traces[i]));
        }
      }
    }

    // Delete orphaned screenshot files (screenshot-N.png / screenshot-N.jpg)
    try {
      const files = fs.readdirSync(outputDir);
      for (const file of files) {
        if (/^screenshot-\d+\.(png|jpg|jpeg)$/.test(file) && !referencedScreenshots.has(file)) {
          try {
    ...
    // Delete orphaned trace files
    const tracesDir = path.join(outputDir, 'traces');
    if (fs.existsSync(tracesDir)) {
      try {
        const files = fs.readdirSync(tracesDir);
        for (const file of files) {
          if (file.endsWith('.zip') && !referencedTraces.has(file)) {
            try {
...
export function mergeHistories(
  ...
  mergedHistory.summaries = mergedHistory
