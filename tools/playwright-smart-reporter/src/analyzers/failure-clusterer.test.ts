...
describe('FailureClusterer', () => {
  ...
  describe('clusterFailures', () => {
    ...
    it('extracts error type from various patterns', () => {
      ...
      for (const tc of testCases) {
        const results = [createTestResult({ status: 'failed', error: tc.error })];
  ...
  describe('assignClusters', () => {
    ...
    it('handles clusters without tests property', () => {
      const results = [createTestResult({ testId: 'test-1', status: 'failed' })];
  ...
  describe('getLargestClusters', () => {
    ...
    it('uses default limit of 5', () => {
      const clusters: FailureCluster[] = Array(10)
        .fill(null)
        .map((_, i) => ({
          id: String(i),
          errorType: `Error ${i}`,
