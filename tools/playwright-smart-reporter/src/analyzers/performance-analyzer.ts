...
import { computePercentile } from '../utils';
...
export class PerformanceAnalyzer {
  ...
  analyze(test: TestResultData, history: TestHistoryEntry[]): void {
    ...
    const avgDuration = Math.round(
      relevantHistory.reduce((sum, e) => sum + e.duration, 0) / relevantHistory.length,
    );
    test.averageDuration = avgDuration;

    // Compute percentiles when ≥3 history entries exist
    const MIN_SAMPLE = 3;
    if (relevantHistory.length >= MIN_SAMPLE) {
      const sorted = relevantHistory.map((e) => e.duration).sort((a, b) => a - b);
      test.p50Duration = Math.round(computePercentile(sorted, 50));
      test.p90Duration = Math.round(computePercentile(sorted, 90));
      test.p95Duration = Math.round(computePercentile(sorted, 95));
    } else {
      test.p50Duration = null;
      test.p90Duration = null;
      test.p95Duration = null;
    }

    // Use p50 as regression baseline when available (more robust than mean)
    const baseline = test.p50Duration ?? avgDuration;

    // Calculate performance trend
    test.performanceTrend = this.getPerformanceTrend(test.duration, baseline);

    // Add detailed performance metrics
    test.performanceMetrics = this.calculateMetrics(test.duration, baseline);
  }
