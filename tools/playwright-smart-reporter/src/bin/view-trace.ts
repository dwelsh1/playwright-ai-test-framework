...
function findTraceUnderRoot(rootDir: string, tracePath: string): string | null {
  ...
  const commonRoots = [root, path.join(root, 'example'), path.join(root, 'blob-reports', 'merged')];
...
async function main(): Promise<void> {
  ...
  if (!fs.existsSync(traceFsPath) || !fs.statSync(traceFsPath).isFile()) {
    ...
    console.error(
      'Tip: pass --dir pointing at the report folder (the folder that contains smart-report.html and ./traces).',
    );
    console.error(
      `Example: npx playwright-smart-reporter-view-trace ${JSON.stringify(options.tracePath)} --dir "./example"`,
    );
    process.exit(1);
  }
